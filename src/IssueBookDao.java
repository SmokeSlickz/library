...
public class IssueBookDao {
	...
	public static boolean checkBook(String bookcallno) { // view
		boolean status = false;
		try {
			Connection con = DB.getConnection(); // con str
			PreparedStatement ps = con.prepareStatement("select * from books where callno=?"); // query
	...
	public static int save(String bookcallno, int studentid, String studentname, String studentcontact) { // new
		int status = 0;
		try {
	...
	public static int updatebook(String bookcallno) { // upd
		int status = 0;
